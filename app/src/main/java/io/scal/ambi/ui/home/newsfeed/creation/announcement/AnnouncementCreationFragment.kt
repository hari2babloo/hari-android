...
class AnnouncementCreationFragment : BaseFragment<AnnouncementCreationViewModel, FragmentAnnouncementCreationBinding>(), ICreationFragment {
    ...
    private fun initAsUserSpinner() {
        ...
        viewModel.dataStateModel
            .toObservable()
            .filter { it is AnnouncementCreationDataState.Data }
            .map { it as AnnouncementCreationDataState.Data }
    ...
    private fun initStateModel() {
        ...
        viewModel.errorStateModel.asErrorState(binding.rootContainer,
                                               ...
                                               {
                                                   when (it) {
                                                       is AnnouncementCreationErrorState.NoError    -> ErrorState.NoError
                                                       is AnnouncementCreationErrorState.Error      -> ErrorState.NonFatalError(it.message)
                                                       is AnnouncementCreationErrorState.ErrorFatal -> ErrorState.FatalError(it.message)
                                                   }
                                               },
